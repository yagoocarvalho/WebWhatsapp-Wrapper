  ...
  (function () {
    function getStore(modules) {
      ...
      for (let idx in modules) {
        ...
        // if (module && (module.findJidFromNumber || (module.default && module.default.findJidFromNumber))) {
        //   console.log(module);
    ...
    if (WAPI.isMultiDeviceVersion()) {
      ...
      WAPI.getChat(id, chat => {
        if (chat) {
          ...
          console.log('Mensagem NÃƒO enviada!');
          done(false);
          return false;
        }
      });
    } else {
      // VersÃ£o antiga do WhatsApp  para um sÃ³ navegador
      try {
        ...
        // Quando ainda nÃ£o Ã© um contato
...
window.WAPI.sendImageOld = function (imgBase64, chatid, filename, caption, done) {
  ...
  console.log("[sendImage] Sending image")
  ...
  return Store.Chat.find(idUser).then((chat) => {
    ...
    mc.processAttachments([{ file: mediaBlob }, 1], chat, 1).then(() => {
      console.log("[sendImage] processAttachments")
